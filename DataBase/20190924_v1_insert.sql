...
-- 테이블 데이터 bank.menu:~99 rows (대략적) 내보내기
DELETE FROM `menu`;
/*!40000 ALTER TABLE `menu` DISABLE KEYS */;
INSERT INTO `menu` (`type`, `name`, `kor_name`, `prnts_name`, `status`, `depth`, `sort`) VALUES
	('check', 'check', '자동이체 조회/변경/취소', 'auto', 'y', 2, 1),
	('check', 'check', '자동이체결과 조회', 'auto', 'y', 2, 2),
	('check', 'register', '자동이체 등록', 'auto', 'y', 2, 0),
	('utlity', 'bauto', '지로자동이체신청/조회/해지', 'bill', 'y', 2, 3),
	('utlity', 'bill', '지로납부', 'utlity', 'y', 1, 1),
	('utlity', 'bpay', '지로요금납부', 'bill', 'y', 2, 0),
	('product', 'checkrn', '조회/입출금', 'deposit', 'y', 2, 0),
	('check', 'check', '조회 · 이체', '', 'y', 0, 0),
	('product', 'check', '조회/입금/해지', 'fund', 'y', 2, 0),
	('product', 'check', '대출계좌조회', 'loan', 'y', 2, 0),
	('check', 'check', '예약이체 처리결과 조회', 'reservation', 'y', 2, 2),
	('check', 'acc', '계좌조회', 'check', 'y', 1, 0),
	('product', 'close', '해지', 'deposit', 'y', 2, 2),
	('product', 'deposit', '예금/신탁', 'product', 'y', 1, 0),
	('check', 'check', '지연이체등록 조회/취소', 'delay', 'y', 2, 1),
	('check', 'result', '지연이체 처리결과 조회', 'delay', 'y', 2, 2),
	('check', 'register', '당행/타행 지연이체 등록', 'delay', 'y', 2, 0),
	('utlity', 'eei', '전기요금', 'bill', 'y', 2, 2),
	('utlity', 'etc', '기타', 'utlity', 'y', 1, 4),
	('product', 'fund', '펀드', 'product', 'y', 1, 2),
	('product', 'guide', '상품안내', 'deposit', 'y', 2, 3),
	...
	('usermanagement', 'apply', '서비스신청', 'accaign', 'y', 2, 1),
	('usermanagement', 'assign', '이용기기등록', 'assign', 'y', 2, 0),
	('usermanagement', 'hidden', '계좌감추기 서비스', 'accounty', 'y', 2, 1),
	('usermanagement', 'info', '서비스안내', 'accaign', 'y', 2, 0),
	('usermanagement', 'release', '이용기기등록 해제', 'assign', 'y', 2, 2),
	('usermanagement', 'search', '등록기기조회/삭제', 'assign', 'y', 2, 1),
	('usermanagement', 'excepip', '예외ip로그인알람서비스', 'blockalarm', 'y', 2, 1),
	('usermanagement', 'outalarm', '해외ip로그인알람서비스', 'blockalarm', 'y', 2, 2),
	('usermanagement', 'outip', '해외 ip차단 신청', 'blockalarm', 'y', 2, 0),
	('usermanagement', 'info', '서비스안내', 'delaytrs', 'y', 2, 0),
	('usermanagement', 'modify', '서비스신청/변경', 'delaytrs', 'y', 2, 1),
	('usermanagement', 'accaign', '신입금계좌지정서비스', 'usermanagement', 'y', 1, 5),
	('usermanagement', 'assign', '이용기기등록서비스', 'usermanagement', 'y', 1, 4),
	('usermanagement', 'blockalarm', '보안차단/알림서비스', 'usermanagement', 'y', 1, 7),
	('usermanagement', 'delaytrs', '지연이체서비스', 'usermanagement', 'y', 1, 6),
	('product', 'mortgage', '상품안내', 'loan', 'y', 2, 3),
	('utlity', 'myut', '나의공과금조회/납부', 'utlaw', 'y', 2, 0),
	('utlity', 'nadu', '국세/관세', 'utlity', 'y', 1, 3),
	('utlity', 'nat', '국세', 'nadu', 'y', 2, 0),
	('product', 'new', '신규', 'deposit', 'y', 2, 1),
	('product', 'new', '신규', 'fund', 'y', 2, 1),
	('utlity', 'patcom', '특허수수료', 'nadu', 'y', 2, 1),
	('product', 'payment', '이자납부원금상환', 'loan', 'y', 2, 1),
	('utlity', 'pen', '범칙금', 'etc', 'y', 2, 0),
	('product', 'pledge', '신용', 'loan', 'y', 2, 2),
	('product', 'product', '금융상품', '', 'y', 0, 1),
	('utlity', 'reserve', '공과금예약납부조회/취소', 'utlaw', 'y', 2, 1),
	('check', 'check', '이체결과조회', 'right', 'y', 2, 0),
	('check', 'check', '예약이체등록 조회/취소', 'reservation', 'y', 2, 1),
	('check', 'register', '예약이체등록', 'reservation', 'y', 2, 0),
	('product', 'search', '상품안내', 'fund', 'y', 2, 2),
	('utlity', 'staco', '국고입력납부', 'nadu', 'y', 2, 3),
	('utlity', 'tariff', '관세', 'nadu', 'y', 2, 2),
	('check', 'right', '즉시이체', 'check', 'y', 1, 1),
	('check', 'reservation', '예약이체', 'check', 'y', 1, 2),
	('utlity', 'tut', '등록금', 'etc', 'y', 2, 1),
	('usermanagement', 'acccs', '평생계좌전환서비스', 'accmanag', 'y', 2, 4),
	('usermanagement', 'bbcc', '통장표시내용변경', 'accmanag', 'y', 2, 6),
	('usermanagement', 'deposit', '자주쓰는입금계좌관리', 'accmanag', 'y', 2, 3),
	('usermanagement', 'anickname', '계좌별명관리', 'accmanag', 'y', 2, 5),
	('usermanagement', 'apw', '계좌비밀번호관리', 'accmanag', 'y', 2, 1),
	('usermanagement', 'asequencecha', '계좌순서변경', 'accmanag', 'y', 2, 2),
	('usermanagement', 'withdacc', '출금계좌관리', 'accmanag', 'y', 2, 0),
	('usermanagement', 'edit', '마이메뉴편집', 'intbank', 'y', 2, 0),
	('usermanagement', 'holdcheck', '계좌정지 신청/조회', 'intbank', 'y', 2, 5),
	('usermanagement', 'limitred', '이체한도감액', 'intbank', 'y', 2, 4),
	('usermanagement', 'unuserel', '장기미사용정지 해제', 'intbank', 'y', 2, 3),
	('usermanagement', 'bagreement', '(은행)개인(신용)정보수집-이용제공동의', 'info', 'y', 2, 2),
	('usermanagement', 'changenum', '입금불능연락처 변경', 'info', 'y', 2, 1),
	('usermanagement', 'infochange', '고객정보 조회/변경', 'info', 'y', 2, 0),
	('usermanagement', 'usermanagement', '사용자 · 보안관리', ' ', 'y', 0, 3),
	('usermanagement', 'cashier', '자기앞수표', 'report', 'y', 2, 1),
	('usermanagement', 'scotp', 'otp', 'report', 'y', 2, 0),
	('usermanagement', 'accmanag', '계좌관리', 'usermanagement', 'y', 1, 2),
	('usermanagement', 'info', '고객정보관리', 'usermanagement', 'y', 1, 0),
	('usermanagement', 'intbank', '인터넷뱅킹관리', 'usermanagement', 'y', 1, 1),
	('usermanagement', 'report', '사고신고', 'usermanagement', 'y', 1, 3),
	('utlity', 'utlaw', '공과금/법원센터', 'utlity', 'y', 1, 0),
	('utlity', 'utlity', '공과금/법원', '', 'y', 0, 2),
	('utlity', 'wase', '상하수도요금', 'lotax', 'y', 2, 3),
	('check', 'auto', '자동이체', 'check', 'y', 1, 3),
...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `postal_code` varchar(50) NOT NULL COMMENT '우편번호',
...
INSERT INTO `user` (`id`, `pw`, `simple_pw`, `name`, `tel`, `gen`, `email`, `job_group`, `addr`, `postal_code`, `status`, `register_date`, `end_date`) VALUES
	('day_0821', '1111', 0, '박이찬', '010-####-#', '', 'TT@green.com', '프로그래머', '', '', '활성', '2019-09-05 08:31:36', NULL),
	('elliottjo', '1234', 7777, '조은환', '010-####-#', '남자', 'TT@green.com', '프로그래머', '인천시', '', '활성', '2019-09-18 00:56:13', NULL),
	('exception', '123456', 921028, '황동규', '010-####-#', '남자', 'TT@green.com', '프로그래머', '안양시', '', '활성', '2019-09-10 11:11:00', NULL);
...
DELETE FROM `user_simple_pw_log`;
...
DELETE FROM `user_transfer_limit`;
