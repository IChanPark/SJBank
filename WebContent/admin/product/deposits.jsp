...
function detail(me) {
	...
	$.ajax({	
		...
		success:function(qqq){
			...
			box +=	"</div></div>";
			box +=	"</div>";
			box +=	"<div class='infoBot'><div class='infoBot_Back' onclick='goMenu(this)' data-menu-name='admin/Product/Deposits'>목록으로</div></div>";
			box +=	"</div>";
			$("#mm").append(box);
			isRun = false;
		},
...
function ev7(){if(!check(/^[\d.]{0,5}[\d]$/,$('.prf_interest'),'5자리 이하 숫자와 .만 입력가능합니다.')){return false;}else{return true;}};
...
function add(){
	...
	box	+=	"<select id='sel_type' onchange='setTax()'><option value='보통'>보통</option><option value='정기'>정기</option></select></div></div>";
	...
	box +=  "<input onfocusout='ev2()' type='text' placeholder='숫자와 ,만 입력해주세요.' id='month'>상품 개월 수를 ,로 구분하여 입력해주세요.</div></div>";
	box +=  "<div class='infoMain_Info'><div class='infoMain_Type'>이자지급방식</div><div class='infoMain_Value'>";
	box +=  "<input type='text' readonly='readonly' id='interest_type' value='월이자지급'>상품 종류 선택시 자동으로 입력됩니다.</div></div>";
...
function modify(me) {
	...
	$.ajax({	
		...
		success:function(qqq){
			...
			box +=  "</div><div class='AdminBot'><div class='AdminButton' onclick='goMenu(this)' data-menu-name='admin/Product/Deposits'>수정취소</div>";
			box +=  "<div class='AdminButton' onclick='modReg()' data-menu-name='product/Product/Deposit'>수정등록</div></div></div>";
			$("#mm").append(box);
			isRun = false;
		},
		error:function(qqq){
